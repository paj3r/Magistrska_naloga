...
def avg_degree_values(values, lookback, positive):
    ...
    for i in range(lookback, len(values)):
        ...
        avg_short_dist_p[i] = np.average([pos.degree[x] for x in pos.nodes])
        avg_short_dist_n[i] = np.average([neg.degree[x] for x in neg.nodes])
...
class VGAvgDegreeStrategy(Strategy):
    ...
    lookback = 20
    def init(self):
        self.avg_deg_vals = self.I(avg_degree_values, self.data.Close, self.lookback, True)
        self.avg_deg_vals_neg = self.I(avg_degree_values, self.data.Close, self.lookback, False)
    ...
    def next(self):
        ...
        if self.short_ma[-1] < self.long_ma[-1]:
            ...
            if (not self.position and self.avg_deg_vals[-2] < self.avg_deg_vals[-1]
                    and self.avg_deg_vals_neg[-2] > self.avg_deg_vals_neg[-1]
...
bt = Backtest(dataframe, VGAvgDegreeStrategy, cash=100000, exclusive_orders=True)
