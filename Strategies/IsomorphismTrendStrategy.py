...
def VG_max_clique_predictions(prices, positive):
    ...
    for i in range(1, len(out)):
        ...
        out[i] = generate_new_node_isomorphism_trend_linear(temp_prices)
    if not positive:
        out = [-x for x in out]
    return out

class IsomorphismTrendStrategy(Strategy):
...
bt = Backtest(dataframe, IsomorphismTrendStrategy, cash=100000,  commission=.002, exclusive_orders=True)
