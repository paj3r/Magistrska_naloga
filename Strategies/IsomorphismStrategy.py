...
def VG_max_clique_predictions(prices, positive):
    ...
    for i in range(1, len(out)):
        ...
        out[i] = generate_new_node_isomorphism(temp_prices)
    if not positive:
        out = [-x for x in out]
    return out

class IsomorphismStrategy(Strategy):
...
bt = Backtest(dataframe, IsomorphismStrategy, cash=100000,  commission=.002, exclusive_orders=True)
