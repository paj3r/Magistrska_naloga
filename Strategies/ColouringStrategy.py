...
def VG_max_clique_predictions(prices, positive):
    ...
    for i in range(1, len(out)):
        ...
        out[i] = generate_new_node_colouring(temp_prices)
    if not positive:
        out = [-x for x in out]
    return out

class ColouringStrategy(Strategy):
...
bt = Backtest(dataframe, ColouringStrategy, cash=100000,  commission=.002, exclusive_orders=True)
